...
def get_completion2(client,prompt, model="gpt-4o-mini", temperature=0, top_p=1.0, max_tokens=1024, n=1, json_output=False):
    if json_output == True:
...
def get_completion(client,prompt, model="gpt-4o-mini", temperature=0, top_p=1.0, max_tokens=256, n=1, json_output=False):
    if json_output == True:
      output_json_structure = {"type": "json_object"}
    else:
      output_json_structure = None
...
def intro():
    ...
    st.write("This app will help you create a new draft of an Award Recommendation Document.")
    st.markdown("""👈 Select an option from the dropdown on the left** to see what this app can do!""")
    st.write(" ")
    st.markdown("**Contact us:**")
    st.write("Please reach out to us if you have any queries.")
    st.write(" ")
        ...
        with st.spinner('Loading...'):
            ...
            prompt = f"""
                    ...
                    **Avoid including any additional text or headers.**
                    ...
                    `{pdf_text}`
                    """
            response = get_completion2(client,prompt)
            ...
            force_font(document,"A.  Aim") #Forces the font to be bold and arial
            #st.write(response)
            ...
            force_font(document,"B.  Background") #Forces the font to be bold and arial
            document.add_paragraph(response)
            ...
            force_font(document,"C.  Procurement Approach") #Forces the font to be bold and arial
            document.add_paragraph(response)
            ...
            force_font(document,"E.  Offers Received") #Forces the font to be bold and arial
            prompt = f"""
            ...
            prompt = f"""
                ...
                `{pdf_text}`
                """
            rownum = get_completion2(client,prompt)
            ...
            force_font(document,"F.  Evaluation")#Forces the font to be bold and arial
            ...
            force_font(document,"G. Price Assessment") #Forces the font to be bold and arial
            #first paragraph
            ...
            #cleaned_dataset = get_completion2(client,prompt)
            ...
            force_font(document,"H.  Recommendation") #Forces the font to be bold and arial
            ...
            if response == "True":
                ...
                `{table_string}`
                """
                provisionalsumtext = get_completion(client,prompt)
            ...
            force_font(document,"I.  Approval") #Forces the font to be bold and arial
            prompt = f"""
...
page_names_to_funcs = {
    "About the App": intro,
    "Processing Award Document": processing_doc,
}
